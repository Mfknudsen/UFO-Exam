        ...
        public static void Main()
        {
            ...
            for (int numberIndex = 1; numberIndex <= 5; numberIndex++)
            {
                ...
                for (int i = 0; i < averageCount; i++)
                {
                    ...
                    Console.WriteLine(
                        $"Vertex count match: {navMeshOptimized.GetVertices().Length == navMeshImport.FVertex()} _ {navMeshImport.FVertex() - navMeshOptimized.GetVertices().Length}");
                    Console.WriteLine(
                        $"Indices count match: {navMeshOptimized.GetIndices().Length == navMeshImport.FIndices()} _ {navMeshImport.FIndices() - navMeshOptimized.GetIndices().Length}");
                    Console.WriteLine(
                        $"Triangle count match: {navMeshOptimized.GetTriangles().Length == navMeshImport.FTriangle()} _ {navMeshImport.FTriangle() - navMeshOptimized.GetTriangles().Length}\n");
